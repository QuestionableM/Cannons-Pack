    ...
    for k, RlgProj in pairs(self.projectiles) do
        if RlgProj and RlgProj.hit then
            ...
            if RlgProj.count > 0 and RlgProj.hit.type ~= "invalid" and RlgProj.hit.type ~= "terrainAsset" and RlgProj.hit.type ~= "terrainSurface" then
                ...
                self.network:sendToClients("client_loadProjectile",{proj.position,proj.velocity,proj.shellEffect,proj.explosionEffect,proj.explosionLevel,proj.explosionRadius,proj.explosionImpulseRadius,proj.explosionImpulseStrength,proj.count,proj.effectToGive})
            end
        end
    end
end
...
function RailgunProjectile.client_onScriptUpdate(self, dt)
    for k, RlgProj in pairs(self.projectiles) do
        ...
        if RlgProj and not RlgProj.hit then
            ...
            local hit, result = _PhysicsRaycast(RlgProj.pos, RlgProj.pos + RlgProj.dir * dt * 1.2)
            ...
            if RlgProj.dir:length() > 0.0001 then
                RlgProj.effect:setRotation(_Vec3GetRotation(sm.vec3.new(1, 0, 0), RlgProj.dir))
            end
