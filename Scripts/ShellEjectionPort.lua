...
ShellEjector.connectionInput = sm.interactable.connectionType.logic
ShellEjector.connectionOutput = sm.interactable.connectionType.none
ShellEjector.poseWeightCount = 1
ShellEjector.colorNormal = sm.color.new(0x8a0038ff)
ShellEjector.colorHighlight = sm.color.new(0xff0067ff)
...
function ShellEjector:server_onFixedUpdate()
    ...
    if parent then
        ...
        if effect_data == nil then
            ...
            if self.interactable.active then
                ...
                table.insert(self.server_shell_queue, effect_data)
            end
        end
    end
    if #self.server_shell_queue > 0 and not self.shell_launch_delay and self.anim.value == 1.0 then
        ...
        table.remove(self.server_shell_queue, 1)
    end
...
function ShellEjector:client_onUpdate(dt)
    ...
    local _ChangedValue = math.min(math.max(self.anim.value + _Changer * dt, 0), 1)
